...
ofxGuiGroup::ofxGuiGroup(){
	...
	spacingNextElement = defaultElementSpacing;
...
void ofxGuiGroup::setWidthElements(float w){
	for(int i=0;i<(int)collection.size();i++){
		...
		if(subgroup!=NULL){
			subgroup->setWidthElements(w*defaultElementIndentation);
		}
	}
	sizeChangedCB();
	generateDraw();
...
void ofxGuiGroup::draw(bool framebufferedMode){
    if( framebufferedMode ){
        if( requiresRerender ){
            ofxBaseGui::draw();
            ...
            for(int i = 0; i < (int)collection.size(); i++){
                collection[i]->draw(framebufferedMode);
            }
        }
    }
    else{
...
void ofxGuiGroup::prerender(){
    // use flag to turn on/off fb mode
    if( useFbo ) {
        ofPushMatrix();
        guiFbo.begin();
        // I've been using the line below for testing, to check ui elements are not being rerendered every frame
        //ofClear(0,0);
        ofTranslate(-b.x,-b.y);
    }
}
...
ofxLabel & ofxGuiGroup::getLabel(string name){
    return getControlType<ofxLabel>(name);
}
...
void ofxGuiGroup::save(){
    saveToFile( filename );
}

void ofxGuiGroup::useFrameBuffer(bool on){
    useFbo = on;
	// TODO isn't this meant to do something?
    for(int i = 0; i < (int)collection.size(); i++){
        //ofxBaseGui* obj = collection[i];
    }
}

void ofxGuiGroup::setFboOnChild(ofxBaseGui* obj){
    ofxGuiGroup* group = dynamic_cast<ofxGuiGroup*>(obj);
    bool isGroup = group != NULL;
	// TODO shouldn't this only do something if isGroup == true? 
    if( obj != NULL ){
        group->useFrameBuffer(useFbo);
    }
}

void ofxGuiGroup::initFbo(){
    guiFbo.allocate(b.width, b.height);
    guiFbo.begin();
    ofClear(0,0);
    guiFbo.end();
}
