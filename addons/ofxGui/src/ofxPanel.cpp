...
void ofxPanel::setVisible(bool visible){
    isVisible = visible;
}

bool ofxPanel::getVisible(){
    return isVisible;
}

void ofxPanel::toggleVisible(){
    isVisible = !isVisible;
}

void ofxPanel::draw(){
    if( isVisible ) ofxBaseGui::draw();
}
...
bool ofxPanel::setValue(float mx, float my, bool bCheck){
	...
	if( bCheck ){
		...
		//cout << mx - grabPt.x <<" " << my - grabPt.y <<endl;
		return true;
	}
	return false;
}
